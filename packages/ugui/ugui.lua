...
---PUBLIC GUI API
-------------------------------------------------------------------------------------------
function ugui.SET_SELECTED_OBJECT(OBJ)
	SELECTED_OBJECT = OBJ
end

function ugui.CLEAR_SCREEN(color, target)
	if not target then target = term.native() end
	target.setBackgroundColor(color)
	target.clear()
end

function ugui.NO_EXIT()
...
function ugui.connectPeripheral(name)
...
--Panel constructor
function ugui.NewPanel(x, y, visible, enabled)
	local panel = {
		addCHILD = function(s, ...)
			local args = {...}
...
--Button constructor
function ugui.NewButton(target, x, y, width, height, text, func, color_bg, color_text, color_used)
	local button = {
...
function ugui.NewCheckBox(target, x, y, width, height, text, func, color_bg, color_text, color_used)
	local chbox = {
		check = false, 
...
function ugui.NewProgressBar(target, x, y, width, height, color_bg, color_used)
	local pbar = {
		step = 0.01,
		progress = 0.0,
...
function ugui.NewTextLine(target, x, y, width, text, func, color_bg, color_text)
	local textline = {
		secure = false,
		_textpos = 1,
		_cursorpos = 1,
...
function ugui.NewTextArea(target, x, y, width, height, text, color_bg, color_text)
	local textarea = {
		draw = function(s)
			if not s.visible then return end
...
--Label constructor
function ugui.NewLabel(target, x, y, text, color_bg, color_text)
	local label = {
		draw = function(s)
			if not s.visible then return end
			local cursorX, cursorY = s.target.getCursorPos()
...
ugui.MainPanel = ugui.NewPanel()

--Event handler. Call this if overwrite os.pullEvent()
function ugui.eventHandler(e)
